...
Map<String, dynamic> mapToFirestore(Map<String, dynamic> data) =>
    data.where((k, v) => k != FirestoreUtilData.name).map((key, value) {
      // Handle GeoPoint
      if (value is MobileLatLng) {
        value = value.toGeoPoint();
      }
      // Handle list of GeoPoint
      if (value is Iterable && value.isNotEmpty && value.first is MobileLatLng) {
        value = value.map((v) => (v as MobileLatLng).toGeoPoint()).toList();
      }
      // Handle Color
      if (value is Color) {
...
List<GeoPoint>? convertToGeoPointList(List<MobileLatLng>? list) =>
    list?.map((e) => e.toGeoPoint()).toList();

extension GeoPointExtension on MobileLatLng {
  GeoPoint toGeoPoint() => GeoPoint(latitude, longitude);
}

extension LatLngExtension on GeoPoint {
  MobileLatLng toLatLng() => MobileLatLng(latitude, longitude);
}
